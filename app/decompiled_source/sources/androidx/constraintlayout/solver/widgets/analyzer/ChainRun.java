    ...
    */
    public void update(Dependency dependency) {
        ...
        while (j < 2) {
            ...
            int numMatchConstraints2 = size2;
            int size3 = 0;
            weights = weights2;
            int numMatchConstraints3 = i5;
            while (size3 < count) {
                WidgetRun run = chainRun.widgets.get(size3);
                if (run.widget.getVisibility() == i) {
                    parent = parent2;
                } else {
                    numVisibleWidgets++;
                    if (size3 > 0 && size3 >= firstVisibleWidget) {
                        numMatchConstraints2 += run.start.margin;
                    }
                    ...
                    if (!treatAsFixed) {
                        ...
                        numMatchConstraints2 += dimension2;
                    }
                    if (size3 < count - 1 && size3 < lastVisibleWidget) {
                        numMatchConstraints2 += -run.end.margin;
                    }
                }
                size3++;
                parent2 = parent;
            ...
            if (numMatchConstraints2 >= distance && numMatchConstraints3 != 0) {
                numVisibleWidgets = 0;
                size2 = 0;
                weights2 = 0.0f;
                j++;
        ...
        if (size2 > distance) {
            position5 = isInRtl3 ? position5 + ((int) (((size2 - distance) / 2.0f) + 0.5f)) : position5 - ((int) (((size2 - distance) / 2.0f) + 0.5f));
        }
            ...
            while (i6 < count) {
                ...
                if (run2.widget.getVisibility() == 8) {
                    ...
                    if (weights <= 0.0f) {
                        ...
                        dimension = (int) ((((distance - size2) * run2.widget.mWeight[chainRun.orientation]) / weights) + 0.5f);
                    }
        ...
        if (i8 == 1) {
            ...
            for (int i10 = 0; i10 < count; i10++) {
                ...
                if (run5.widget.getVisibility() == 8) {
                    ...
                    if (isInRtl) {
                        position7 = position3 - dimension5;
                    } else {
                        position7 = position3 + dimension5;
                    }
                    if (isInRtl) {
            ...
            float bias = chainRun.orientation == 0 ? chainRun.widget.getHorizontalBiasPercent() : chainRun.widget.getVerticalBiasPercent();
            if (isInRtl) {
                bias = 1.0f - bias;
            }
            int gap3 = (int) (((distance - size2) * bias) + 0.5f);
            gap3 = (gap3 < 0 || numMatchConstraints > 0) ? 0 : 0;
            if (isInRtl) {
